# -*- coding: utf-8 -*-
"""Capstone_Parallel.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1xKR3kQb3khjIdCZd7yVrAhtba-JkohRo
"""
...
h_a = h_a.astype(np.double)
h_b = h_b.astype(np.double)
h_v = h_v.astype(np.double)
#alpha = alpha.astype(np.float32)
#theta = alpha.astype(np.float32)
...
cuda.memcpy_htod(d_v, h_v)

mod = SourceModule("""
  #include <stdio.h>
  #include <math.h>
  __global__ void deviceAdd(double * __restrict__ d_c, const double * __restrict__ d_a, const double * __restrict__ d_b, double * __restrict__ d_v, const int length, const int epochs)
  {
    int i,j;
    double temp = 0;
    double sum=0;
    ...
    for(i = 0; i<epochs;i++){
      ...
      if (tid >= length) return;
        d_c[tid] = (d_b[tid]-(1/(1+expf(-1*temp*d_a[tid]))))*d_a[tid];
        //d_c[tid] = 1/(1+expf(-1*d_v[0]*d_a[tid]));
        sum = 0;
        for(j=0;j<length;j++){
          sum= sum+d_c[j];
        }
        temp = temp + d_v[1]*(sum/length);
    }
    d_c[0] = temp;
...
start_e.record()
deviceAdd(d_c, d_a, d_b,d_v,np.int32(length),np.int32(epochs), block = blockDim, grid = gridDim)
end.record() 
end.synchronize()
secs = start_e.time_till(end) * 1e-3
...
print("\nParallel Time--- %s seconds ---" % (time.time() - parallel_time))
##################################################################################################################################################################################
run = True
while (run):
